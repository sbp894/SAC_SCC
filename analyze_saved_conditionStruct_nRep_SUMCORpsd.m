            ...
            for nRepVar=1:length(uniq_nReps)
                ...
                for windowVar=1:length(uniq_windows)
                    ...
                    if numel(curIND)~=1
                        ...
                        yyaxis left;
                        plot(conditionStruct(curIND).N_freqVEC, 20*log10(conditionStruct(curIND).N_PSDenv),'linewidth', lw);
                        ylabel('power (dB)');
                        ylimLeft=ylim;
                        
                        yyaxis right;
                        plot(conditionStruct(curIND).uR_freqVEC, 20*log10(conditionStruct(curIND).uR_PSDenv),'linewidth', lw);
                        xlim([0 300]);
                        xlabel('freq (Hz)');
                        ylabel('power (dB)');
                        ylimRight=ylim;
                        yRange=max(range(ylimLeft), range(ylimRight));
                        
                        yyaxis right;
                        ylim([mean(ylimRight)-.5*yRange mean(ylimRight)+.5*yRange]);
                        grid on;
                        yyaxis left;
                        ylim([mean(ylimLeft)-.5*yRange mean(ylimLeft)+.5*yRange]);
                        grid on;
