%%
% Created by SP (11 July, 2017)
% To see if results based on Hilbert Transforma and SUMCOR of neanrate
% yield diffent results. 
% Testing because: 
%         - Christoph's model (based on Hilbert) says in SAM noise, SNRenv
%         mostly come from higher modulation frequencies. 
%         - n_sEPSM's model (based on SUMCOR) says in SAM noise, SNRenv
%         mostly come from 1 and 2 Hz modulation frequencies. 
...
for stimVar=1:length(All_stims)
    ...
    legstr(1:2)={'tSUMCOR', 'tHilbert'};
    legstr2(1:4)={'SN-tSUMCOR', 'SN-tHilbert','N-tSUMCOR', 'N-tHilbert'};
    if params.useNeural
    ...
    for srVar=1:length(all_SRtype)
        ...
        for nRepVar=1:length(all_nReps)
            ...
            [n_PowerMod_SN, t_ad_PowerMod_SN, t_hilbert_PowerMod]=fun_getpMOD(params, AN, hFig);
            subplot(length(all_SRtype),2,2*srVar-1);
            hold on;
            if nRepVar==1
                plot(t_ad_PowerMod_SN, [ hFig.linestyles{1} 's'], 'color', co(1,:), 'linewidth', hFig.lw);
                plot(t_hilbert_PowerMod, [ hFig.linestyles{1} 's'], 'color', co(2,:), 'linewidth', hFig.lw);
            end
            if params.useNeural
            ...
            t_ua_Se=(t_hilbert_PowerMod-t_ua_PowerMod_N)./t_ua_PowerMod_N;
            t_ua_Se(t_ua_Se<0)=0;
        ...
        title(sprintf('%sSR, SNRenv=%.2f (t-SUMCOR) and %.2f (t-Hilbert)', srLetters(params.SRtype), nansum(t_ad_Se.^2), nansum(t_ua_Se.^2)));
        set(gca, 'XTick', 1:7, 'xticklabel',  params.strModFreqs);
    end
